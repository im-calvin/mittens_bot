...
def get_en_list():
    ...
    for member_block in file_content:
        en_list.extend(member_block.decode("utf-8").split(','))
...
def timezone_convert(time_list, timezone):
    ...
    new_date_list = [dt.datetime.strptime(t, '%H:%M').replace(
        year=year, month=month, day=day) for t in time_list]
    new_date_list = list(map(lambda x: pytz.timezone(
        "Asia/Tokyo").localize(x), new_date_list))
    ...
    new_date_list = tuple(
        map(lambda x: x.astimezone(new_timezone), new_date_list))
